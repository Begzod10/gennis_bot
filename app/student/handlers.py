...
async def get_payments_list(message: Message):
    ...
    try:
        ...
        await message.answer("❌ To‘lovlar ro‘yhati olinmadi.")
        return
...
async def handle_test_results(message: Message):
    ...
    try:
        ...
        await message.answer("❌ Test natijalari olinmadi.")
        return
...
async def handle_online_test_results(message: Message):
    ...
    try:
        ...
        await message.answer("❌ Onlayn test natijalari olinmadi.")
        return
...
async def get_davomatlar_royxati(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Davomatlar olinmadi.")
        return
...
async def get_baholar(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Baholar olinmadi.")
        return
...
async def handle_month_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer("❌ Ma'lumotlar olinmadi.")
        return

    if not tables:
        await callback.message.answer(f"⚠️ {mode.capitalize()} topilmadi.")
        return

    text = f"📅 <b>{student.name}, {month} oyi:</b>\n\n"
    for table in tables:
        text += f"🔷 <b>{table.get('subject', 'Nomaʼlum fan')} ({table.get('name', '')})</b>\n"
        text += f"👨‍🏫 O'qituvchi: <i>{table.get('teacher', '')}</i>\n"

        if mode == "attendance":
            text += f"📚 Darslar:\n"
            for att in table.get("attendances", []):
                status_icon = "✅" if att.get("ball_status", 0) in [1, 2] else "❌"
                text += f"  🔹 {att.get('day', '?')} {status_icon}\n"
        else:
            text += f"📈 O'rtacha ball: <b>{table.get('average_ball', 0)}</b>\n"
            for score in table.get("score", []):
                text += f"  🔹 {score.get('day', '?')} ✅\n"
