...
async def handle_test_results(message: Message):
    ...
    for group in test_results:
        text += f"👥 <b>Guruh:</b> {group['name']}\n📚 <b>Fan:</b> {group['subject']}\n👨‍🏫 <b>O'qituvchi:</b> {group['teacher']}\n"
        ...
        if not tests:
            ...
            for result in tests:
                text += (
                    ...
                    f"✅ <b>Natija:</b> {result['percentage']} ({result['true_answers']} ta to'g'ri javob)\n"
                    f"🎯 <b>Test:</b> {result['test_info']['name']} (Daraja: {result['test_info']['level']})\n"
                )
                text += "━" * 20 + "\n"
        text += "═" * 25 + "\n\n"
...
async def handle_month_selection(callback: CallbackQuery, state: FSMContext):
    ...
    if mode == "attendance":
        ...
        for table in tables:
            text += f"🔷 <b>{table['subject']} ({table['name']})</b>\n👨‍🏫 O'qituvchi: <i>{table['teacher']}</i>\n📈 O'rtacha ball: {table['average_ball']}\n"
            if not table['score']:
                text += "⚠️ Baholar mavjud emas.\n"
            else:
                text += "📚 Darslar bo‘yicha baholar:\n"
...
def get_tests():
    try:
        resp = requests.get(API, headers=HEADERS(GENNIS_TOKEN))
