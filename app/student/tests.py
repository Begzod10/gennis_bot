...
from .keyboards import student_basic_reply_keyboard_test_type

GENNIS_TOKEN = os.getenv("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..kyqMRFUS8AcPDqMJ8yLD0duqw0Q0wwiGgGgAkcfdKlc")
...
@student_routers.message(F.text == "🏁 Onlayn test yechish")
...
@student_routers.message(F.text == "⬅️ Orqaga")
async def back_to_menu(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("👆 Iltimos, testni tanlang:", reply_markup=student_basic_reply_keyboard_test_type)
...
async def select_test(message: types.Message, state: FSMContext):
    ...
    await message.answer("🧠 Test yuklanmoqda...", reply_markup=ReplyKeyboardRemove())
    ...
    if not questions:
        await message.answer("❗️ Bu testda savollar topilmadi.", reply_markup=student_basic_reply_keyboard_test_type)
        return
...
async def send_question(message: types.Message, state: FSMContext):
    ...
    buttons.append([KeyboardButton(text="❌ Testdan chiqish")])
    keyboard = ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)
...
@student_routers.message(TestStates.question_number, F.text == "❌ Testdan chiqish")
...
async def finish_test(message: types.Message, state: FSMContext):
    ...
    print(
        f"Natija: {message.from_user.id}, {message.from_user.username}, {score}/{total} ({percent}%), {answered_questions}")

    await message.answer(
        f"✅ Test tugadi!\nSiz {score}/{total} to‘g‘ri javob berdingiz.\n📊 Foiz: {percent}%\n📄 Javob bergan savollar:\n" + "\n".join(
            answered_questions),
