...
async def start_test_handler(message: types.Message, state: FSMContext):
    await message.answer("🧠 HTML testi boshlanmoqda...\n⏳ 10 soniyadan keyin birinchi savol chiqadi!")
    await state.update_data(index=0, correct=0, answers={})

    # Countdown
    msg = await message.answer("⏳ <b>10 soniya qoldi...</b>")
    for i in range(10, 0, -1):
        await msg.edit_text(f"⏳ <b>{i} soniya qoldi...</b>")
...
async def send_question(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"🧩 <b>{q['q']}</b>", reply_markup=builder.as_markup())
